...
from lambdas.create_batch_lambda_handler import CreateBatches
...
def test_create_new_batch_id_raises_exception():
    with pytest.raises(Exception):
        create_batch_obj = CreateBatches()
        create_batch_obj.create_new_batch_id(None)


@mock_sqs
def test_push_batch_id_to_queue():
    ...
    push_message_obj.push_batch_id_to_queue(generated_batch_id)
    assert True


@mock_sqs
def test_push_batch_id_to_queue_raises_exception():
    if os.environ.get('SQS_CURATED_BATCHES_QUEUE_ARN', None):
        os.environ.pop('SQS_CURATED_BATCHES_QUEUE_ARN')
...
def test_create_batch():
    with pytest.raises(Exception):
        ...
        assert CreateBatches().create_batch() is None
...
def test_create_batch_current_batch_id_not_empty_string():
    ...
    def mock_push_batch_id_to_queue(*args, **kwargs):
        pass
    ...
    create_batches = CreateBatches()
    create_batches.get_latest_batch = mock_get_latest_batch
    create_batches.push_batch_id_to_queue = mock_push_batch_id_to_queue
    create_batches.create_new_batch_id = mock_create_new_batch_id
